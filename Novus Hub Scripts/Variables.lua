...
pcall(function()
	if isfile and writefile and readfile then
		...
		local function SetWebhook()
			writefile("NovusHubWebhooking.txt", CurrentTime)
			print("[Novus Hub] Debug: Webhook Delay Set at "..CurrentTime)
			Webhook = GlobalWebhook
		end

		if not isfile("NovusHubWebhooking.txt") then
			SetWebhook()
		elseif tonumber(readfile("NovusHubWebhooking.txt")) < CurrentTime - 7200 then
			SetWebhook()
		else
			Webhook = nil
		end
...
print("[Novus Hub] Debug: Detected Executor: "..getexploit())
...
task.spawn(function()
	...
	pcall(SendMessage, "[Novus Hub] Data: Novus Hub was executed by "..((Player.Name ~= Player.DisplayName and Player.DisplayName) or "Unknown.."..Player.Name:sub(-2, -1)).." on "..game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name.." "..VCurrentVersion.." using "..getexploit(), "Execution")
end)
...
local function CreateWindow()
	...
	local Window = Rayfield:CreateWindow({
		Name = "Novus Hub - "..game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name.." - "..VCurrentVersion,
		LoadingTitle = "Novus Hub",
		LoadingSubtitle = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name,
		ConfigurationSaving = {
			Enabled = true,
			FolderName = "NovusHubConfig",
			...
			Callback = function(Value)
				if Value then
					...
					if queueteleport then
						queueteleport('loadstring(game:HttpGet("https://raw.githubusercontent.com/kxzyInt/Roblox-Scripts/main/Script%20Hub%20-%20Novus%20Hub.lua"))()')
					end
				end
			end,
		})
					...
					pcall(function()
						if isfile and writefile and readfile then
							...
							local function SetSuggestionsWebhook()
								...
								local success, result = pcall(SendMessage, "[Novus Hub] Data: "..((Player.Name ~= Player.DisplayName and Player.DisplayName) or "Unknown.."..Player.Name:sub(-2, -1)).." suggested "..Text.." on "..game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name, "Suggestion")
								if success then
									Notify("Successfully Sent Suggestion", 5)
									writefile("NovusHubWebhooking2.txt", CurrentTime)
									print("[Novus Hub] Debug: Webhook Delay Set at "..CurrentTime)
								else
							...
							if not isfile("NovusHubWebhooking2.txt") then
								SetSuggestionsWebhook()
							elseif tonumber(readfile("NovusHubWebhooking2.txt")) < CurrentTime - 86400 then
								SetSuggestionsWebhook()
							else
								Webhook = nil
